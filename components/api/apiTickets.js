...
const getInstructivo = (from, index) => {
  ...
  if (
    ...
    numericIndex <= 0 ||
    numericIndex > instructivos.length
  ) {
...
const computerInfo = (from, option) => {
  if (ticket[from].computers[option - 1] && option !== "0") {
    ...
    const banderaShort =
      selected.bandera.length > 3
        ? selected.bandera.substring(0, 3)
        : selected.bandera;
...
const sendEmail = async (from) => {
  ...
  if (
    ticket[from].mailAttachments &&
    ticket[from].mailAttachments.length !== 0
  ) {
...
const getManagers = async (from, id) => {
  ...
  managers.botusers.forEach((botuser) => {
    if (botuser.manager === true) {
      if (
        (botuser.email && botuser.area === ticket[from].zone) ||
        botuser.area === "G"
      ) {
...
const getUsers = (from) => {
  if (ticket[from].users.length === 1) {
    ...
    const usersString = ticket[from].users
      .map((user, index) => `${index + 1}. ${user.info}`)
      .join("\n");
    return usersString;
  }
};
